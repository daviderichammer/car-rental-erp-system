        ...
        <div class="modal-content">
            ...
            <form id="addReservationForm" onsubmit="safeCall((e) => addReservation(e))">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="reservationPickupDate" placeholder="YYYY-MM-DD HH:MM (e.g., 2025-07-26 10:00)">
                </div>
                <div class="form-group">
                    <label>Return Date & Time</label>
                    <input type="text" id="reservationReturnDate" placeholder="YYYY-MM-DD HH:MM (e.g., 2025-07-28 10:00)">
                </div>
                <div class="form-group">
    ...
    <script>
                    ...
                    try {
                        return `
                            <div class="card">
                                ...
                                <p><strong>Category:</strong> ${vehicle.category?.category_name || vehicle.category || 'N/A'}</p>
                                <p><strong>Daily Rate:</strong> ${vehicle.daily_rate ? `$${vehicle.daily_rate}` : 'Contact for pricing'}</p>
        ...
        async function addReservation(event) {
            try {
                ...
                // Validate dates - improved validation for text inputs
                const pickup = new Date(pickupDate.replace(' ', 'T'));
                const returnDateTime = new Date(returnDate.replace(' ', 'T'));
                
                if (isNaN(pickup.getTime()) || isNaN(returnDateTime.getTime())) {
                    throw new Error('Please enter valid dates in YYYY-MM-DD HH:MM format');
                }
                ...
                const reservationData = {
                    ...
                    vehicle_category_id: "2", // Use valid category ID
            ...
            try {
                ...
                const maintenanceCards = maintenanceList.map(maintenance => {
                    try {
                        ...
                        const cost = maintenance.estimated_cost ? `$${maintenance.estimated_cost}` : 'TBD';
            ...
            try {
                ...
                const transactionCards = transactionList.map(transaction => {
                    try {
                        ...
                        const date = transaction.transaction_date ? new Date(transaction.transaction_date).toLocaleDateString() : 'Date not available';
