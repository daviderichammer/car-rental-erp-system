    ...
    <script>
        ...
        function displayVehicles(vehicleList) {
            ...
            const vehicleCards = vehicleList.map(vehicle => `
                <div class="card">
                    ...
                    <div class="card-actions">
                        <button class="btn btn-info" onclick="editVehicle('${vehicle.vehicle_id}')">Edit</button>
                        <button class="btn btn-warning" onclick="scheduleMaintenanceVehicle('${vehicle.vehicle_id}')">Maintenance</button>
                        <button class="btn btn-danger" onclick="archiveVehicle('${vehicle.vehicle_id}')">Archive</button>
                    </div>
                </div>
            `).join('');
                ...
                if (select) {
                    select.innerHTML = '<option value="">Select Vehicle</option>' + 
                        vehicles.map(vehicle => 
                            `<option value="${vehicle.vehicle_id}">${vehicle.make} ${vehicle.model} (${vehicle.license_plate})</option>`
                        ).join('');
                }
            });
        }
        ...
        function displayCustomers(customerList) {
            ...
            const customerCards = customerList.map(customer => `
                <div class="card">
                    ...
                    <p><strong>Phone:</strong> ${customer.user.phone_number || customer.phone || 'N/A'}</p>
                    <p><strong>License:</strong> ${customer.driver_license_number}</p>
                    <p><strong>Date of Birth:</strong> ${customer.user.date_of_birth || 'N/A'}</p>
                    <p><strong>Status:</strong> <span class="status-badge status-${customer.user.status || 'active'}">${customer.user.status || 'Active'}</span></p>
                    <div class="card-actions">
                        <button class="btn btn-info" onclick="editCustomer('${customer.customer_id}')">Edit</button>
                        <button class="btn btn-warning" onclick="viewCustomerHistory('${customer.customer_id}')">View History</button>
                        <button class="btn btn-danger" onclick="deactivateCustomer('${customer.customer_id}')">Deactivate</button>
                    </div>
                </div>
            `).join('');
            ...
            if (select) {
                select.innerHTML = '<option value="">Select Customer</option>' + 
                    customers.map(customer => 
                        `<option value="${customer.customer_id}">${customer.user.first_name} ${customer.user.last_name}</option>`
                    ).join('');
            }
        }
        ...
        function searchCustomers(query) {
            const filtered = customers.filter(customer => 
                (customer.user.first_name + ' ' + customer.user.last_name).toLowerCase().includes(query.toLowerCase()) ||
                customer.user.email.toLowerCase().includes(query.toLowerCase()) ||
                (customer.user.phone_number || customer.phone || '').includes(query) ||
                customer.driver_license_number.toLowerCase().includes(query.toLowerCase())
            );
        ...
        function displayReservations(reservationList) {
            ...
            const reservationCards = reservationList.map(reservation => `
                <div class="card">
                    ...
                    <p><strong>Customer:</strong> ${reservation.customer?.user?.first_name || 'N/A'} ${reservation.customer?.user?.last_name || ''}</p>
                    <p><strong>Vehicle:</strong> ${reservation.vehicle_category?.category_name || ''}</p>
                    <p><strong>Start Date:</strong> ${new Date(reservation.pickup_datetime).toLocaleDateString()}</p>
                    <p><strong>End Date:</strong> ${new Date(reservation.return_datetime).toLocaleDateString()}</p>
                    <p><strong>Total Amount:</strong> $${reservation.total_estimated_cost || 'N/A'}</p>
        ...
        function searchReservations(query) {
            const filtered = reservations.filter(reservation => 
                ...
                (reservation.customer?.user?.first_name + ' ' + reservation.customer?.user?.last_name || '').toLowerCase().includes(query.toLowerCase()) ||
                (reservation.vehicle_category?.category_name || '').toLowerCase().includes(query.toLowerCase())
            );
        ...
        async function addReservation(event) {
            ...
            // Get selected vehicle to extract category_id
            const selectedVehicleId = document.getElementById("reservationVehicle").value;
            ...
            const reservationData = {
                ...
                pickup_location_id: "a80b995b-c256-4499-8f54-3b9cce0d9689",
                return_location_id: "a80b995b-c256-4499-8f54-3b9cce0d9689",
            ...
            try {
                ...
                if (response.ok) {
                    const result = await response.json();
                    alert("Reservation created successfully! Reservation #: " + result.reservation.reservation_number);
                    closeAddReservationModal();
                    loadReservations();
                } else {
                    const error = await response.json();
                    alert("Error creating reservation: " + (error.error || "Unknown error"));
                }
            } catch (error) {
        ...
        async function loadMaintenance() {
            try {
                const response = await fetch(`${API_BASE}/maintenance/schedules`);
                const data = await response.json();
        ...
        function searchMaintenance(query) {
            const filtered = maintenanceRecords.filter(maintenance => 
                (maintenance.service_type || '').toLowerCase().includes(query.toLowerCase()) ||
                (maintenance.service_notes || '').toLowerCase().includes(query.toLowerCase()) ||
        ...
        async function addMaintenance(event) {
            ...
            const maintenanceData = {
                ...
                service_type: document.getElementById('maintenanceType').value,
                description: document.getElementById('maintenanceDescription').value,
                scheduled_date: document.getElementById("maintenanceDate").value,
                estimated_cost: parseFloat(document.getElementById('maintenanceCost').value)
            };
        ...
        async function loadTransactions() {
            try {
                const response = await fetch(`${API_BASE}/financial/payments`);
                const data = await response.json();
        ...
        async function addTransaction(event) {
            ...
            const transactionData = {
                payment_type: document.getElementById('transactionType').value,
